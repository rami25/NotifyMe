...
export class AuthService {
  ...
  /**
   * Native platforms (Android/iOS) auto-initialize the GoogleAuth plugin
   * from capacitor.config.ts. The web target does not — it needs this
   * explicit call with the same client ID, or signIn() throws. Safe to
   * call multiple times; only runs once.
   */
  private async ensureGoogleAuthInitialized(): Promise<void> {
    if (this.googleAuthInitialized || Capacitor.getPlatform() !== 'web') {
  ...
  async signInWithGoogle0(): Promise<{ ok: true } | { ok: false; message: string }> {
    ...
    if (!idToken) {
      // Missing idToken almost always means serverClientId (capacitor.config.ts)
      // or googleWebClientId (environment.ts) isn't set to a valid Web
      // application OAuth Client ID — see the comments on both.
      await GoogleAuth.signOut();
      return { ok: false, message: "Sign-in didn't complete correctly. Try again." };
    }
